...
### SF10 — the actual second attempt, deliberately more conservative than SF12
Per explicit follow-up request to re-attempt max range "but don't break anything this time" — reframed as a real conversation about risk rather than repeating the same gamble: **SF12's root cause was never actually confirmed**, only reverted, so going straight back to it would have been re-betting on the same unverified assumption that just cost multiple bricked boards. SF10 was chosen instead as a smaller, better-reasoned step: its time-on-air (~944ms for this packet, computed the same way as SF12's ~3.8s above: ~100ms preamble + ~844ms payload at BW125/CR4:5/explicit header/CRC on) keeps its symbol duration under the 16ms threshold that requires RadioLib to enable low-data-rate-optimization for SX126x — the leading (still unconfirmed) suspect for what hung at SF11/SF12. SF10 sidesteps that specific mechanism entirely, without claiming to have found or fixed the actual root cause.
**What shipped this time**: spreading factor 10 (not 12) on both sender and receiver, `LORA_TX_INTERVAL_MS` ~2s (real headroom above the ~944ms airtime, not the ~4.5s SF12 needed), and the SAME async-TX rework from the SF12 attempt reintroduced (`startTransmit()`/`setPacketSentAction()`/`finishTransmit()`, `loraTxInFlight`/`loraTxDoneFlag`/`checkLoraTxComplete()`) — SF10's ~944ms airtime is still long enough that a blocking `transmit()` would stall `loop()` (GPS/ESP-NOW included) for most of a second every cycle, so the same reasoning that justified async TX at SF12 still applies at SF10, just with smaller numbers. ESP-NOW keeps its own separate `ESPNOW_TX_INTERVAL_MS` (200ms/5Hz, unaffected).
**Still not confirmed working on real hardware as of this pass** — same honest caveat as every radio-behavior claim in this file that hasn't been bench-tested: this is a smaller, more conservative bet than SF12, reasoned from the LDRO-threshold theory, not a proven-safe change. **Test on ONE board with a Serial Monitor attached before flashing others** — the SF12 incident affected multiple physical boards simultaneously specifically because it wasn't validated on a single unit first before wider deployment; don't repeat that pattern even for a more conservative change. **The underlying range problem is a range/risk tradeoff now, not solved outright** — SF10 gives real range over SF7 without SF12's full extreme, and without (as far as currently known) SF12's hang risk, but "as far as currently known" is doing real work in that sentence until it's actually been run on hardware.
...
## Current State (V3.6 — packet shrunk 98→81 bytes for less LoRa airtime, SF10 range attempt still unconfirmed on hardware)

- **Packet shrunk: `hdop` float→byte, `esc_mode`/`esc_state` strings→1-byte codes** — 98 → **81 bytes**, ~862ms time-on-air at SF10 (down from ~985ms, ~12% less). See the dedicated rule above. **Not yet verified on real hardware.**
...
- **A move to SF12 for range was tried, flashed to real hardware, confirmed to hang the board (no serial output at all, on multiple units), and fully reverted** — see the ⚠️ rule above for the full incident.
- **SF10 is the second attempt** — real range gain over SF7, deliberately staying below the low-data-rate-optimization threshold suspected (not confirmed) to have caused the SF12 hang. See the dedicated rule above. Brings back the async-TX rework (non-blocking `startTransmit()`/`finishTransmit()`, separate `LORA_TX_INTERVAL_MS`/`ESPNOW_TX_INTERVAL_MS`) since SF10's ~944ms airtime is still too long for a blocking `transmit()` call to be safe.
- **Not yet confirmed working on real hardware** — test on ONE board with a Serial Monitor attached before flashing others; don't repeat the SF12 incident's mistake of flashing multiple boards before validating one.
- Dual-radio: LoRa (SX1262, 915 MHz, SF10/BW125, 22 dBm, ~2s interval) + ESP-NOW (200ms/5Hz, its own separate interval)
- Sensor loop @ 5 Hz (`SENSOR_INTERVAL_MS = 200`), RPM recalculated @ 5 Hz (`RPM_CALC_INTERVAL_MS = 200`), ESP-NOW TX @ 5 Hz — LoRa TX is the one exception, at ~2s per the SF10 change above
