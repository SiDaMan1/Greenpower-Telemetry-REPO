...
//           Same RF settings as the sender (915 MHz, SF12, BW125, sync 0xF3
//           — SF12 for max range, up from SF7; a packet now takes ~3.8s of
//           airtime, so the sender only transmits roughly once every ~4.5s
//           now, not 5x/sec) — see config.h, which must stay in sync with
//           the sender's copy.
//
...
void setup() {
    ...
    int loraState = radio.begin(
        ...
        125.0,                // bandwidth kHz
        12,                   // spreading factor — SF12, max range (was SF7); MUST match the sender's copy exactly or packets won't decode (see config.h's own "must stay in sync" rule)
        5,                    // coding rate 4/5
        LORA_SYNC_WORD,       // 0xF3
    ...
    if (loraState != RADIOLIB_ERR_NONE) {
        ...
        if (rxState != RADIOLIB_ERR_NONE) {
            ...
            loraReady = true;
            Serial.println("[OK]   SX1262  915 MHz  SF12  BW125  22dBm  listening...");
        }
    }
