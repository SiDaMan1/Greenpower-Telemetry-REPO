...
// Preamble length, in symbols — must match the sender's copy of this define
// exactly. See greenpower_sender/config.h's own comment for why this is 6
// (Semtech's documented minimum for reliable SX126x sync) instead of the
// earlier default of 8 — pure latency trim, ~0.5ms/packet at SF7/BW500.
#define LORA_PREAMBLE_SYMBOLS  6


// ════════════════════════════════════════════════════════════════════
//  SHARED TELEMETRY PACKET  (binary, sender → receiver over LoRa)
//  Must be identical to greenpower_sender/config.h — 33 bytes, no padding.
// ════════════════════════════════════════════════════════════════════
...
// escStateToCode() in greenpower_sender.ino), which is 5 total values
// needing 3 bits; mode(2 bits)+state(3 bits) = 5 bits doesn't fit flags'
// 4 spare bits (0-3 are GPS/IMU/CUR/ESC valid) without actually losing
// that UNKNOWN distinction, so esc_state_code stays its own full byte
// below instead of taking a real, if tiny, precision hit to save one
// more byte.
