...
//  Must be identical to greenpower_sender/config.h — 42 bytes, no padding.
// ════════════════════════════════════════════════════════════════════
...
typedef struct __attribute__((packed)) {
    ...
    uint8_t  esc_ramp_pct;       // ramp/re-engage tracker %, from ESC controller
                             ...
                             // ramp(1) = 42 bytes (was 73 with float fields — ~42% smaller payload,
                             // which is what actually buys back LoRa airtime; see "Current State"
                             // in this folder's CLAUDE.md for the compression pass writeup).
