...
### ✅ CORRECTION — the SF10/SF12 "hangs" documented in this section were a flashing mistake, NOT a real radio.begin()/spreading-factor incompatibility
**Per direct user confirmation**: the boards failing to boot/respond at SF10 and SF12 were caused by an incorrect flashing procedure at the time, not a genuine hardware or RadioLib limitation with this board at higher spreading factors. **The caution below (and in `greenpower_sender/CLAUDE.md`) against trying SF8-SF12 no longer applies** — there's no known reason this board can't run any valid spreading factor. The rules below are kept as real history (they're what led to the async-TX rework and the SD-card SPI bug being found), just don't treat "SF10/SF12 hangs this board" as still true when reasoning about a future range/SF change.

### (HISTORICAL — see ✅ CORRECTION above) SF12 was tried, confirmed to hang THIS device (not just the sender), and has been fully reverted to SF7
...
### (HISTORICAL — see ✅ CORRECTION above) SF10 ALSO confirmed to hang this board on real hardware — reverted to SF7 a second time
...
**Range is now genuinely reduced from every prior configuration in this project's history** — the explicit, accepted tradeoff, not an oversight. If range complaints resurface, the fix used at the time was lowering bandwidth back down (500→250→125) rather than raising spreading factor, since SF10/SF12 were believed to hang this board — **since corrected: that was a flashing mistake, not a real SF limitation (see the ✅ CORRECTION note above)**, so a higher SF is a legitimate option to reconsider too, not just bandwidth.
...
## Current State (V2.11 — ⚠️ REAL bug found and fixed, matching greenpower_sender's V4.14: Heltec V4's external antenna front-end (GC1109) was never enabled)

- **Matching `greenpower_sender`'s V4.14 pass** — see that file's CLAUDE.md for the full diagnosis (a real user report of -120dBm RSSI at 15ft, traced to this board's external GC1109 antenna front-end chip needing explicit GPIO enable that RadioLib has no way to provide on its own, confirmed via two independent sources: a Reddit report and a real GitHub PR to the MeshCore project using the identical pins).
- **New `FEM_EN_PIN` (2, CSD) / `FEM_CPS_PIN` (46, CPS)**, both set `OUTPUT`+`HIGH` in `setup()` immediately before `radio.begin()`. **No pin conflict on this board** — unlike the sender, which had to temporarily disable its RTC to free GPIO2, this receiver never used GPIO2 or GPIO46 for anything. `FEM_CPS_PIN`'s TX-power-mode purpose doesn't really apply to a receive-only board, but asserting it anyway is harmless and keeps both boards' antenna-enable code identical.
- **Not yet verified on real hardware** — expect RSSI to jump from ~-120dBm to somewhere in the -40 to -60dBm range at 15ft once both boards are reflashed with this fix, per this board's own advertised -137dBm sensitivity spec.
- **Brace balance confirmed 24/24** after this edit.
...
- **New computed time-on-air ≈ 160ms** at SF7/BW62.5 (was ~170ms with the field) — this project no longer measures airtime directly, only the Semtech-formula estimate in comments.
- **Not yet reflashed/verified on real hardware.**

## Current State (V2.4 — bandwidth pushed to 62.5kHz, matching greenpower_sender's V4.7)

- **Matching `greenpower_sender`'s V4.7 pass** — `radio.begin()`'s bandwidth is `62.5` now (was `125.0`), SF unchanged at 7. Per explicit follow-up to maximize range while staying under the 200ms update budget. New airtime ≈ 170ms, ~30ms of real margin left — close to the practical ceiling for a bandwidth-only push at SF7 within this budget; a higher SF is the next lever if more range is ever wanted (no longer avoided on hang-risk grounds — see V2.3's correction above).
- **MUST match the sender's copy exactly**, updated in the same pass — a mismatch means packets simply won't decode.
- **Not yet reflashed/verified on real hardware.**

## Current State (V2.3 — CORRECTION: the SF10/SF12 hangs were a flashing mistake, not a real hardware limit)

- **Matching `greenpower_sender`'s V4.6 correction**: per direct user confirmation, the SF10/SF12 hangs documented in this file's ⚠️ rules were caused by an incorrect flashing procedure at the time, not a genuine `radio.begin()` incompatibility. See the ✅ CORRECTION note near the top of this file. Nothing about the actual shipped radio settings changed — SF7/BW125 (V2.2 below) is still what's configured; this is a documentation correction so a future SF/range decision isn't made on a false premise.

## Current State (V2.2 — bandwidth back to 125kHz for RANGE, matching greenpower_sender's V4.5)

- **Matching `greenpower_sender`'s V4.5 pass** — `radio.begin()`'s bandwidth is `125.0` now (was `500.0`), SF unchanged at 7. Per explicit request to gain range without changing the 200ms update cadence — this project's own packet-compression work had left ~180ms of the 200ms budget unused at BW500 (~21ms airtime), and BW125 spends most of that slack (~85ms airtime, still ~115ms of margin) on real receiver sensitivity instead.
- **MUST match the sender's copy exactly, same as every other radio.begin() parameter** — this device's own `radio.begin()` bandwidth argument was updated in the same pass, not left to drift.
- **Not yet reflashed/verified on real hardware.**

## Current State (V2.1 — decodes+forwards the sender's real, MEASURED airtime: `airtime_ms_x10`, 42→44 bytes)

- **Matching `greenpower_sender`'s V4.4 pass** — per explicit follow-up ("is there no way you can actually measure the lora time on air"), the sender now timestamps its own TX-complete interrupt (`micros()`, real hardware timing) and writes the measured duration into the packet, one transmission of lag behind (see that file's CLAUDE.md for the full timing mechanism and why the lag exists). This device just decodes it: `airtimeMs = pkt.airtime_ms_x10 / PKT_SCALE_AIRTIME`, computed once alongside the other decoded fields and reused for both the pretty dump (new `Airtime : X.X ms (measured, previous TX)` line) and the `JSON:` line's new `"airtime_ms"` key.
- **Packet grew 42 → 44 bytes** — `radio.implicitHeader(sizeof(telemetry_packet_t))` (see the fix note below) automatically tracks this; no manual length update needed on this side.
- **A real compile error caught and fixed this pass**: `radio.implicitHeader(sizeof(pkt))` in `setup()` — this device's `pkt` is declared LOCAL to the packet-handling function it's actually used in, not visible from `setup()` at all (`'pkt' was not declared in this scope`). Fixed to `sizeof(telemetry_packet_t)` (the type, not a variable) — also more correct going forward, since it now automatically tracks the struct's real size on any future field change rather than needing to match whatever `pkt` variable happens to be in scope.
- **telemetry_web needs no server.js/dashboard-logic changes beyond adding `airtime_ms` to `METRICS`/`CSV_COLUMNS`** — the JSON key is new, but its shape (a plain number, real ms unit) follows the exact same pattern every other field already uses. See that project's own CLAUDE.md for its side of this (a new Link-page card/chart, replacing what used to be a purely computed, static "expected" figure with this real one).
- **Not yet reflashed/verified on real hardware.**

## Current State (V2.0 — implicit header + trimmed preamble: ~20ms airtime)

- **Matching `greenpower_sender`'s V4.3 pass** — `radio.begin()` now uses `LORA_PREAMBLE_SYMBOLS` (6, down from 8, shared via `config.h`) instead of a hardcoded `8`, and calls `radio.implicitHeader(sizeof(pkt))` right after `begin()` succeeds, before `startReceive()`.
- **This is now a hard coupling, not just a style match**: implicit-header receivers don't get told the payload length in-band (that's the entire point — it's the overhead being removed), so this device's `implicitHeader()` call MUST use the exact same length the sender's does, or every packet will fail to decode with no useful error — the radio will report `[OK]` and appear to be listening normally either way. If packets ever stop decoding right after a firmware update on either side, check this pairing first.
- **Combined airtime ≈ 20ms/packet** — see `greenpower_sender/CLAUDE.md`'s matching entry for the full breakdown and the two further cuts (dropping `epoch_time`, going below SF7) that were considered and deliberately not applied without asking.
- **Not yet reflashed/verified on real hardware.**

## Current State (V1.9 — packet compressed to 42 bytes: fixed-point instead of float)

- **`telemetry_packet_t` recompressed — 73 → 42 bytes.** Explicit request ("compress the data ... so it transmits super fast near zero"). Every float field except `latitude`/`longitude` is now a scaled `int16`/`uint16` (`speed_mph_x10`, `temp_f_x10`, `batt_volt_x100`, `current_a_x100`, `pitch_deg_x100`, `accel_g_x1000`, etc.), and the three `esc_*_pct` fields are now whole-percent `uint8`. Scale factors and the `PKT_TEMP_NO_READING` sentinel (an int16 has no NaN, so this replaces the old float `isnan()` check for "sensor disconnected") live in `config.h`'s "Fixed-point compression" block — matching `greenpower_sender/config.h` exactly, as always.
- **This device's pretty-dump and `JSON:` block now decode every field back to a human float before printing/serializing** (`pkt.field_x10 / PKT_SCALE_X`, done once per packet into local `speedMph`/`tempF`/`battVolt`/... variables, then reused for both the human dump and the JSON line so they can't drift apart). **The `JSON:` line's own shape is unchanged** — same keys, same real units (mph, volts, °F, ...) — so `receiver_agent`/`telemetry_web` need zero changes; the compression is entirely invisible past this device's own decode step. Same reasoning as the ESC mode/state numeric-code compression documented above (`### Compact JSON — ESC as numeric codes...`) — this is just that idea applied to the rest of the packet.
- **Recomputed time-on-air at SF7/BW500: ~36ms → ~22ms per packet** — pure payload-size reduction, no range cost (lat/lon, the only range-sensitive precision fields, were left as float on purpose).
- **Not yet reflashed/verified on real hardware.**
