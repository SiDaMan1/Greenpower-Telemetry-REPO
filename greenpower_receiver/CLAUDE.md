...
## Current State (V1.1 — SF12 range attempt reverted, back to known-good SF7)

- **A move to SF12 (to match a sender-side range attempt) was tried, confirmed to hang this device (no serial output at all, on multiple boards), and fully reverted** — `radio.begin()`'s spreading-factor argument is back to `7`. See the ⚠️ rule above before attempting this again; the hang happens in `radio.begin()` itself, not in any TX/RX runtime behavior, so "this device only listens" turned out not to matter.
