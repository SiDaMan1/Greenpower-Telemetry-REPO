...
**If it does NOT hang this time**: that would be a genuinely new, useful data point (three consistent hangs at SF10/SF12/BW125 vs. one working case at SF12/BW62.5 would suggest bandwidth, not spreading factor, is the actual variable that matters) — worth updating this file's history immediately if that's what happens, rather than assuming the mechanism is understood.
...
## Current State (V1.6 — THIRD range attempt: SF12+BW62.5, explicitly requested despite two prior confirmed hangs)

- **SF12 + BW62.5 now flashed, deliberately, as a third range attempt** — `radio.begin()` is `12`/`62.5` (was `7`/`125.0`). Both SF10 and SF12 (at BW125) have ALREADY hung this exact board's `radio.begin()`, confirmed twice; this combination has no new information avoiding either failure and was chosen anyway per an explicit user decision made with that history laid out first. **Genuinely expected to reproduce the same hang** — see the dedicated ⚠️ rule above for the full reasoning, the computed ~6.9s time-on-air, and what it would mean if this attempt DOESN'T hang (a real, useful new data point). **Not yet confirmed on real hardware.**
