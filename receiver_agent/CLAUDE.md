...
| `installer/GreenpowerAgent.wxs` | **WiX v3 source — builds `GreenpowerAgentSetup.msi`, the real installer this folder currently ships as** (Start Menu shortcut with right-click Uninstall, entry in Add/Remove Programs, no admin needed). See its own header comment for the build command and why `.msi` specifically (not `.exe` — see the rule below, this is load-bearing, not a preference) | **Yes — keep in sync with what `setup.bat` does; see the rule below** |
...
**`Package Platform="x64"` (with matching `candle.exe -arch x64`)** — added after the user reported the installer window flashing twice right on opening it, before any interaction. Candidate fix, not independently confirmed as THE cause (couldn't observe the actual flash in this environment): without an explicit Platform, WiX compiles x86, and double-clicking an x86 `.msi` on 64-bit Windows can involve a brief re-launch under the SysWOW64 `msiexec.exe` before the real one takes over. If the flash is still there after this, that theory was wrong and it needs a different diagnosis — don't assume it's fixed without the user confirming.
**An explicit "Uninstall Greenpower Receiver Agent" shortcut sits next to the app shortcut in the Start Menu folder** — not relying on Windows' right-click-a-shortcut → Uninstall context menu option, which the user confirmed did NOT appear for this app even after a normal install. Real, sourced explanation, not a guess: a plain (non-advertised) MSI shortcut often carries no "uninstall metadata" for Windows' shortcut-resolution heuristic to find, and Microsoft's own guidance is that Add/Remove Programs — not a Start Menu right-click — is the guaranteed uninstall path for a reason. Switching to MSI-advertised shortcuts (the traditional fix for the right-click case specifically) was considered and not attempted: advertised shortcuts typically need a FILE-based component KeyPath to resolve correctly, which directly conflicts with the registry-based KeyPath ICE38 requires for a per-user-profile component (see the rule above) — not a combination this file has verified works together. The explicit shortcut (`Target="[SystemFolder]msiexec.exe" Arguments="/x [ProductCode] /qb"`) sidesteps needing that resolution at all — `[ProductCode]` is a real MSI built-in property, not something authored elsewhere in this file.
...
## Current State (V1.4 — switched to a WiX .msi installer after the .exe was found blocked on managed devices)
