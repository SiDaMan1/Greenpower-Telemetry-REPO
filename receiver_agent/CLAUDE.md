...
**Only one Start Menu shortcut (the app launcher) — no separate explicit "Uninstall" shortcut.** One was added briefly (`Target="[SystemFolder]msiexec.exe" Arguments="/x [ProductCode] /qb"`) after an initial report that right-click-to-uninstall wasn't appearing, then removed again at the user's explicit request once real-machine testing showed Windows' right-click → Uninstall (which routes to Settings → Apps) does appear — though the user later clarified that specific working right-click was observed on the *explicit uninstall shortcut itself*, not confirmed on the plain launcher shortcut that remains now. Bottom line, independent of any shortcut behavior: this is a real registered MSI product, so **Settings → Apps → Installed apps** (or Control Panel → Programs and Features) will always list it for uninstall regardless of Start Menu shortcut behavior — that's the guaranteed path, not something to re-litigate with another shortcut unless the user specifically asks again.
